...
public class TKDoanhThuNVQL extends JFrame implements ActionListener {
	...
	public TKDoanhThuNVQL() throws SQLException, Exception {
		...
		lblAvatar1.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/avatar1.jpg")));
		lblAvatar1.setBounds(50, 20, 39, 39);
		pnlHeader.add(lblAvatar1);

		JLabel lblAvatar2 = new JLabel("");
		lblAvatar2.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/avatar1.jpg")));
		...
		btnTaoHoaDon.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/Add to basket.png")));
		...
		btnQLSP.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/Favourites.png")));
		...
		btnQLKH.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/User.png")));
		...
		btnQLNV.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/Users.png")));
		...
		btnTK.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/Price list.png")));
		btnTK.setRadius(50);
		...
		lblTong.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/Price list.png")));
		...
		btnTim.setRadius(39);
		btnTim.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/Search.png")));
		btnTim.setBounds(556, 55, 39, 39);
		...
		lblThng.setBounds(273, 64, 46, 30);
		...
		comboBoxThang.setBounds(329, 64, 67, 28);
		pnlDoanhThu.add(comboBoxThang);

		JLabel lblNam = new JLabel("Năm:");
		lblNam.setFont(new Font("Dialog", Font.PLAIN, 15));
		lblNam.setBounds(406, 64, 39, 30);
		pnlDoanhThu.add(lblNam);

		comboBoxNam = new JComboBox();
		comboBoxNam.setModel(new DefaultComboBoxModel(new String[] { "2022", "2023", "2024" }));
		comboBoxNam.setFont(new Font("Tahoma", Font.PLAIN, 18));
		comboBoxNam.setBounds(452, 64, 86, 28);
		pnlDoanhThu.add(comboBoxNam);

		JLabel lblThngKDoanh = new JLabel("Thống kê doanh số bán hàng của nhân viên");
		lblThngKDoanh.setBackground(new Color(128, 64, 0));
		lblThngKDoanh.setFont(new Font("Dialog", Font.BOLD, 23));
		lblThngKDoanh.setBounds(0, 0, 516, 39);
		...
		JLabel lblTopNV = new JLabel("Doanh thu của tất cả nhân viên trong tháng");
		lblTopNV.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/Best.png")));
...
//		TableRowSorter<TableModel> sorter = new TableRowSorter<TableModel>(tableTop.getModel());
...
//		sorter.setSortKeys(sortKeys);
		...
		lblKoDoanhThu.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/Delete.png")));
		...
		menuDoanhThu.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/Price list.png")));
		menuBar.add(menuDoanhThu);

		menuNhanVien = new JMenuItem("Nhân viên");
		menuNhanVien.setIcon(new ImageIcon(TKDoanhThuNVQL.class.getResource("/User.png")));
		menuNhanVien.setForeground(new Color(230, 230, 230));
		menuNhanVien.setBackground(new Color(84, 80, 75));
		menuNhanVien.setFont(new Font("Segoe UI", Font.PLAIN, 15));
		menuNhanVien.setHorizontalAlignment(SwingConstants.CENTER);
		menuNhanVien.setSelected(true);
		...
		menuDoanhThu.addActionListener(this);
	...
	private void updateTable2(int thang, int nam) throws Exception {
		model2.setRowCount(0);
		for (NhanVien nv : nv_dao.getDsNhanVien()) {
			double dt = 0;
			for(HoaDon hd : hd_dao.timMaNV_thang_nam(nv.getMaNV(), thang, nam)) {
				dt += hd.tinhTongTien();
			}
	...
	private void updateTable3(int thang, int nam) throws Exception {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o.equals(menuDoanhThu)) {
			TKDoanhThuQL frame = null;
			try {
				frame = new TKDoanhThuQL();
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			frame.setVisible(true);
		...
		if (o.equals(btnTim)) {
			...
			try {
				...
				for (HoaDon hd : hd_dao.timThang_nam(thang, nam)) {
					if (hd.getNhanVien().getMaNV().equals(ma)) {
						ds.add(hd);
					}
				}
				updateTable1(ds);
				updateTable2(thang, nam);
				updateTable3(thang, nam);
			} catch (Exception e1) {
				e1.printStackTrace();
			}
			double tong = 0.0;
